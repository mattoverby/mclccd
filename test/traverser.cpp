...
Eigen::Vector3d
point_triangle_barys(const Eigen::Vector2d& p,
...
class PointInTriangle : public mcl::ccd::BVHTraverse<double, 2>
{
  public:
    ...
    PointInTriangle(const Eigen::Vector2d& pt_, const Eigen::MatrixXd& V_, const Eigen::MatrixXi& F_)
        : pt(pt_)
        , V(V_)
        , F(F_)
    {
    ...
    bool intersectVolume(const VolumeType& v) { return v.contains(pt); }

    bool intersectObject(const ObjectType& o)
    {
        Eigen::Vector3i f = F.row(o.idx);
        Eigen::Vector3d b = point_triangle_barys(pt, V.row(f[0]), V.row(f[1]), V.row(f[2]));
        if (b.minCoeff() >= 0) {
            in_tri.emplace_back(o.idx);
        }
