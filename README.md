...
Apart from continuous and discrete collision detections, traversers can be used to walk down the BVH. Two functions need to be implemented in the derived class to determine if the branch should be traversed (intersectVolume) and to process the leaf (intersectObject). See [test/traverser.cpp](test/traverser.cpp) for a complete example.
...
public:
    ...
    bool intersectObject(const ObjectType &o) {
        const Triangle &tri = triangles[o.idx];
